...
(frameId) => { // eslint-disable-line no-unused-expressions
  const rod = {
    ...
    overlay (id, left, top, width, height, msg) {
    ...
    elementOverlay (id, msg) {
      ...
      const msgDiv = document.createElement('div')
      div.id = id
      div.style = `position: absolute; z-index:2147483647; border: 2px dashed red;
        border-radius: 3px; box-shadow: #5f3232 0 0 3px; pointer-events: none;
        box-sizing: border-box;
        left: ${left}px;
        top: ${top}px;
      ...
      div.appendChild(msgDiv)
      this.offsetParent.appendChild(div)

      if (window.innerHeight < msgDiv.offsetHeight + box.top + box.height) {
        msgDiv.style.top = -msgDiv.offsetHeight - 2 + 'px'
      }
    },
